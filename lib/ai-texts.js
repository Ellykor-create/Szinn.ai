...
// ── De generatie-call ────────────────────────────────────────────────────────
async function generateBlueprintTexts(ctx, lang, client) {
  client = client || new Anthropic();
  const facts = buildFacts(ctx);
  const intakeSummary = buildIntakeSummary(ctx.intake);
  ...
  const stream = client.messages.stream({
    ...
    messages: [{
      ...
      content: [
        {
          type: 'text',
          text: `Hier is de volledige, exact berekende data van de cliënt.\n\n${facts}\n\n${intakeSummary}\n\nSchrijf nu alle tekstvelden van de Alignment Blueprint voor deze persoon.`,
  ...
  if (message.stop_reason === 'refusal') {
    throw new Error('AI-generatie geweigerd (stop_reason: refusal).');
  }
  if (message.stop_reason === 'max_tokens') {
    throw new Error('AI-generatie afgekapt (max_tokens bereikt).');
  }
  const text = message.content.filter(b => b.type === 'text').map(b => b.text).join('');
  const parsed = JSON.parse(text);
  return { texts: parsed, usage: message.usage };
}
