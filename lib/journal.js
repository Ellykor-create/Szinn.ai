...
//   { morning: { intention, feeling, powers[], gratitude[3], at },
//     evening: { gratitude[3], clarity 0-10, shadows[], note, at } }
...
function mergeJournalEntry(entry, body) {
  ...
  if (body.morning && typeof body.morning === 'object') {
    ...
    entry.morning = {
      ...
      gratitude: cleanList(m.gratitude, 3, 300),
  ...
  if (body.evening && typeof body.evening === 'object') {
    ...
    entry.evening = {
      gratitude: cleanList(e.gratitude, 3, 300),
...
if (require.main === module) {
  ...
  let e = mergeJournalEntry(null, { morning: { intention: ' focus ', feeling: 'rustig', energy: 'Sinn', powers: ['Intuïtie'], gratitude: ['a', 'b', 'c', 'd'] } });
  assert.strictEqual(e.morning.intention, 'focus');
  assert.strictEqual(e.morning.energy, 'Sinn', 'energie-keuze bewaard');
  assert.strictEqual(e.morning.gratitude.length, 3, 'max 3 dankbaarheden');
  const morningAt = e.morning.at;
  e = mergeJournalEntry(e, { evening: { gratitude: ['x'], clarity: '14', shadows: [3, '99', 'q'], note: 'n' } });
  ...
  const junk = mergeJournalEntry({}, { morning: { intention: 42, powers: 'nee', gratitude: null } });
