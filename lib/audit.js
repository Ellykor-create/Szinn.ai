...
// expected: optionele verwachte aantallen uit laag 1, bv. { aspects: 9 }
function auditTexts(ai, expected = {}) {
  const problems = [];
  ...
  try {
    ...
    for (const k of ['sun','moon','mercury','venus','mars','jupiter','saturn','uranus','neptune','pluto','northNode','southNode','chiron','mc']) {
      need(filled(q[k]), `astrology.qualities.${k} leeg`);
    }
    if (typeof expected.aspects === 'number') {
      need((ai.astrology?.aspects || []).length === expected.aspects,
        `astrology.aspects: ${(ai.astrology?.aspects || []).length} i.p.v. ${expected.aspects}`);
    }
    ...
    need((ai.energy?.elements || []).length === 4, `energy.elements: ${(ai.energy?.elements || []).length} i.p.v. 4`);
    need(filled(ai.energy?.modality?.body), 'energy.modality leeg');
    need(filled(ai.calendar?.intro), 'calendar.intro leeg');
    need((ai.calendar?.months || []).length === 6, `calendar.months: ${(ai.calendar?.months || []).length} i.p.v. 6`);
    for (const [i, m] of (ai.calendar?.months || []).entries()) {
      need(filled(m.body) && (m.bullets || []).length === 3, `calendar.months[${i + 1}] onvolledig`);
    }
...
function auditHTML(html, { clientName, birthDate, inputText } = {}) {
  const problems = [];
  const need = (cond, msg) => { if (!cond) problems.push(msg); };
  ...
  // "Barry" is de demo-referentieklant. Alleen flaggen als lek: níét wanneer de
  // naam legitiem in de eigen intake van deze klant voorkomt (eigen naam, of een
  // vragenlijst-antwoord dat iemand Barry noemt), anders is het een false positive.
  const nameIsBarry = (clientName || '').trim().toLowerCase() === 'barry';
  const inputHasBarry = /\bbarry\b/i.test(inputText || '');
  if (clientName && !nameIsBarry && !inputHasBarry) {
