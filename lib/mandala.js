...
// Compacte mandala voor het dashboard (300×300, lichte huisstijl):
// Flower of Life, langzaam roterend, Merkaba gericht op de Noordknoop en
// de planeten als stippen op hun exacte ecliptische graden.
function generateMiniMandalaSVG(chart) {
  const C = 150, P = chart.planets;
  const ang = (lon) => (lon - 90) * Math.PI / 180;
  const at = (lon, r) => [C + r * Math.cos(ang(lon)), C + r * Math.sin(ang(lon))];
  const f = (n) => n.toFixed(1);
  const out = [];

  out.push(`<svg viewBox="0 0 300 300" xmlns="http://www.w3.org/2000/svg" role="img">`);
  out.push(`<defs><style>.fol{fill:none;stroke:#C9A96E;stroke-width:.6;opacity:.5}.mk{fill:none;stroke:#A67C3A;stroke-width:1.1;opacity:.85}.pl{fill:#8a6630}.plr{fill:none;stroke:#E4DBC8;stroke-width:.8}</style></defs>`);

  // Flower of Life (19 cirkels) in een traag roterende groep
  out.push(`<g><animateTransform attributeName="transform" type="rotate" from="0 ${C} ${C}" to="360 ${C} ${C}" dur="120s" repeatCount="indefinite"/>`);
  const R = 36;
  out.push(`<circle class="fol" cx="${C}" cy="${C}" r="${R}"/>`);
  for (let i = 0; i < 6; i++) {
    const a = i * 60 * Math.PI / 180;
    out.push(`<circle class="fol" cx="${f(C + R * Math.cos(a))}" cy="${f(C + R * Math.sin(a))}" r="${R}"/>`);
  }
  for (let i = 0; i < 12; i++) {
    const a = i * 30 * Math.PI / 180;
    const r2 = (i % 2 === 0) ? 2 * R : R * Math.sqrt(3);
    out.push(`<circle class="fol" cx="${f(C + r2 * Math.cos(a))}" cy="${f(C + r2 * Math.sin(a))}" r="${R}"/>`);
  }
  out.push(`<circle class="fol" cx="${C}" cy="${C}" r="120"/></g>`);

  // Merkaba: apex van de gouden driehoek wijst naar de Noordknoop
  const nnLon = P.northNode ? P.northNode.lon : 0;
  const tri = (baseLon, cls) => {
    const pts = [0, 120, 240].map(d => at(baseLon + d, 74.4).map(f).join(',')).join(' ');
    return `<polygon class="${cls}" points="${pts}"/>`;
  };
  out.push(tri(nnLon, 'mk'));
  out.push(tri(nnLon + 180, 'mk'));

  // Planetenring met stippen op de exacte graden
  out.push(`<circle class="plr" cx="${C}" cy="${C}" r="103.2"/>`);
  for (const key of ['sun','moon','mercury','venus','mars','jupiter','saturn','uranus','neptune','pluto','northNode','chiron']) {
    const p = P[key];
    if (!p || p.sign === '?') continue;
    const [x, y] = at(p.lon, 103.2);
    out.push(`<circle class="pl" cx="${f(x)}" cy="${f(y)}" r="${key === 'northNode' ? 3 : 2.2}"/>`);
  }
  out.push('</svg>');
  return out.join('');
}

module.exports = { generateMandalaSVG, generateMiniMandalaSVG };
