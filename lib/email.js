...
const FROM = () => process.env.EMAIL_FROM || 'SZINN <noreply@szinn.ai>';
...
async function sendAdminAlert({ orderId, error, attempts }) {
  ...
  if (!to) { console.error(`[admin-alert, geen ADMIN_EMAIL] ${orderId}: ${error}`); return { skipped: true }; }
  const inner = `
  ...
  return sendEmail({ to, subject: `⚠️ SZINN blueprint-generatie mislukt: ${orderId}`, html: layout(inner) });
}

function esc(s) { return String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

module.exports = { sendEmail, sendAccountEmail, sendReadyEmail, sendAdminAlert };
