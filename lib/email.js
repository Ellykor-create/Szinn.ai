...
async function sendAccountEmail({ to, name, tempPassword, isNewAccount, lang = 'nl' }) {
  ...
  // Persoonlijke intakelink — de vragenlijst laadt opgeslagen antwoorden vanzelf
  // zodra je ingelogd bent, dus geen token in de URL nodig.
  const intakeUrl = `${SITE()}${en ? '/intake-en' : '/intake'}`;
  ...
  const inner = en
    ? heading(`Thank you for your order, ${esc(name)}.`) +
      p('If you have <b style="font-weight:500;">already completed the intake questionnaire</b>, your personal SZINN Alignment Blueprint is on its way — this can take a few hours and you will receive an email the moment it is ready.') +
      p("Haven't filled it in yet? Complete your personal intake questionnaire here and we'll get started for you.") +
      button(intakeUrl, 'Complete your intake questionnaire') +
      (isNewAccount ? creds : '') +
      divider() +
      p('Your dashboard is already yours to explore.') +
      button(`${SITE()}/portaal`, 'Open your dashboard') +
      softLine('You were never lost, you were remembering.') +
      signature(lang)
    : heading(`Bedankt voor je bestelling, ${esc(name)}.`) +
      p('Als je het <b style="font-weight:500;">intakeformulier al hebt ingevuld</b>, is jouw persoonlijke SZINN Alignment Blueprint onderweg — dit kan een aantal uur duren en je ontvangt een e-mail zodra hij klaarstaat.') +
      p('Nog niet ingevuld? Vul hier je persoonlijke intakeformulier in, dan gaan we voor je aan de slag.') +
      button(intakeUrl, 'Vul je intakeformulier in') +
      (isNewAccount ? creds : '') +
      divider() +
      p('Je dashboard is alvast van jou om te verkennen.') +
...
// ── Mail — wachtwoord opnieuw ingesteld (aangevraagd via "wachtwoord vergeten") ─
async function sendPasswordResetEmail({ to, name, newPassword, lang = 'nl' }) {
  const en = lang === 'en';
  const loginUrl = `${SITE()}/portaal/inloggen`;
  const creds = credsBox(
    en
      ? `<b style="font-weight:500;">Your new password.</b><br>Email: ${code(to)}<br>Password: ${code(newPassword)}<br><span style="font-size:12px;color:#8a7860;">Log in with this password — you can change it later in your profile.</span>`
      : `<b style="font-weight:500;">Je nieuwe wachtwoord.</b><br>E-mail: ${code(to)}<br>Wachtwoord: ${code(newPassword)}<br><span style="font-size:12px;color:#8a7860;">Log hiermee in — je kunt het daarna wijzigen in je profiel.</span>`
  );
  const inner = en
    ? heading(`Your new password, ${esc(name)}.`) +
      p('You requested a new password for your SZINN portal. Use the password below to log in. Your previous password no longer works.') +
      creds +
      button(loginUrl, 'Go to the portal') +
      infoBox("Didn't request this? Someone may have entered your email address. Your account is safe as long as no one can read your email — simply reset again or contact us if in doubt.") +
      softLine('You were never lost, you were remembering.') +
      signature(lang)
    : heading(`Je nieuwe wachtwoord, ${esc(name)}.`) +
      p('Je hebt een nieuw wachtwoord aangevraagd voor je SZINN-portaal. Log in met het onderstaande wachtwoord. Je vorige wachtwoord werkt niet meer.') +
      creds +
      button(loginUrl, 'Naar het portaal') +
      infoBox('Heb je dit niet aangevraagd? Dan heeft iemand mogelijk je e-mailadres ingevuld. Zolang niemand bij je e-mail kan is je account veilig — vraag gerust opnieuw een herstel aan of neem contact op bij twijfel.') +
      softLine('Je was nooit verloren, je was aan het herinneren.') +
      signature(lang);
  return sendEmail({
    to,
    subject: en ? 'Your new SZINN password' : 'Je nieuwe SZINN-wachtwoord',
    html: layout(inner, { lang, preheader: en ? 'Your new password is inside.' : 'Je nieuwe wachtwoord staat hierin.' }),
  });
}
...
// Interne melding: er staat nieuwe feedback klaar in het admin-dashboard.
// Los van ALERT_TO: feedback gaat naar info@szinn.nl (env override mogelijk).
const FEEDBACK_TO = () => process.env.FEEDBACK_ALERT_EMAIL || 'info@szinn.nl';
async function sendFeedbackAlert({ name, email, rating, message, lang } = {}) {
  const stars = rating ? '★'.repeat(rating) + '☆'.repeat(5 - rating) : '—';
  const inner = heading('Nieuwe feedback ontvangen') +
    infoBox(`Van: <b style="font-weight:500;">${esc(name || 'Anoniem')}</b>${email ? ` &middot; ${esc(email)}` : ''}<br>Beoordeling: ${stars}<br>Taal: ${esc((lang || 'nl').toUpperCase())}`) +
    `<pre style="background:#F6F1E8;border-radius:6px;padding:12px;font-size:13px;white-space:pre-wrap;font-family:${SANS};color:#2c2622;">${esc(message)}</pre>` +
    p(`Bekijk alle feedback in het <a href="${SITE()}/admin" style="color:#A67C3A;">admin-dashboard</a>.`);
  return sendEmail({ to: FEEDBACK_TO(), subject: '💬 Nieuwe SZINN feedback', html: layout(inner, { lang: 'nl' }) });
}

function esc(s) { return String(s ?? '').replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;'); }

// ── Dagelijkse reading (zelfde inhoud als de WhatsApp-variant, per e-mail) ─────
async function sendDailyReadingEmail({ to, name, theme, focus, lang = 'nl' }) {
  const en = lang === 'en';
  const firstName = (name || '').trim().split(/\s+/)[0] || (en ? 'there' : 'daar');
  const inner = en
    ? heading(`Your reading for today, ${esc(firstName)}.`) +
      infoBox(`Today's theme: <b style="font-weight:500;">${esc(theme)}</b><br>Focus: ${esc(focus)}`) +
      button(`${SITE()}/portaal`, 'Open your dashboard') +
      softLine('You were never lost, you were remembering.')
    : heading(`Je reading voor vandaag, ${esc(firstName)}.`) +
      infoBox(`Thema van vandaag: <b style="font-weight:500;">${esc(theme)}</b><br>Focus: ${esc(focus)}`) +
      button(`${SITE()}/portaal`, 'Open je dashboard') +
      softLine('Je was nooit verloren, je was aan het herinneren.');
  return sendEmail({
    to,
    subject: en ? 'Your SZINN reading for today 🌙' : 'Je SZINN reading van vandaag 🌙',
    html: layout(inner, { lang, preheader: en ? `Today: ${theme}` : `Vandaag: ${theme}` }),
  });
}
...
module.exports = {
  ...
  sendPasswordResetEmail, sendDailyReadingEmail, sendFeedbackAlert,
};
