...
async function sendDraftEmail({ to, name, tempPassword, lang = 'nl' }) {
  const continueUrl = `${SITE()}${lang === 'en' ? '/intake-en' : '/intake'}`;
  const loginUrl    = `${SITE()}/szinn-portal/pages/login.html`;
  if (lang === 'en') {
    const inner = `
      <p>Dear ${esc(name)},</p>
      <p>Your answers have been <strong>saved</strong>. Take your time — you can continue the questionnaire whenever it suits you.</p>
      <p>An account has been created for you. Log in with:</p>
      <p style="background:#F0EBE2;border-radius:6px;padding:14px 18px">
        Email: <strong>${esc(to)}</strong><br>Password: <strong>${esc(tempPassword)}</strong></p>
      ${btn(continueUrl, 'Continue your questionnaire')}
      <p style="font-size:13px;color:#8a7a5c">Open the questionnaire while logged in (via <a href="${loginUrl}" style="color:#A67C3A">your dashboard</a>) and your saved answers will be loaded automatically.</p>`;
    return sendEmail({ to, subject: 'Your SZINN questionnaire is saved — continue anytime', html: layout(inner) });
  }
  const inner = `
    <p>Beste ${esc(name)},</p>
    <p>Je antwoorden zijn <strong>opgeslagen</strong>. Neem de tijd — je kunt de vragenlijst afmaken wanneer het jou uitkomt.</p>
    <p>Er is een account voor je aangemaakt. Je kunt inloggen met:</p>
    <p style="background:#F0EBE2;border-radius:6px;padding:14px 18px">
      E-mail: <strong>${esc(to)}</strong><br>Wachtwoord: <strong>${esc(tempPassword)}</strong></p>
    ${btn(continueUrl, 'Ga verder met je vragenlijst')}
    <p style="font-size:13px;color:#8a7a5c">Open de vragenlijst terwijl je ingelogd bent (via <a href="${loginUrl}" style="color:#A67C3A">je dashboard</a>) en je opgeslagen antwoorden worden automatisch ingeladen.</p>`;
  return sendEmail({ to, subject: 'Je SZINN vragenlijst is opgeslagen — ga verder wanneer je wilt', html: layout(inner) });
}
...
module.exports = { sendEmail, sendAccountEmail, sendDraftEmail, sendReadyEmail, sendAdminAlert, sendNewOrderEmail };
