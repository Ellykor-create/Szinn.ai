...
// Hoofdfunctie. deps = { loadDB, saveDB, blueprintStore } uit de function-context.
async function runGeneration(orderId, deps) {
  const { loadDB, saveDB, blueprintStore } = deps;
  ...
  const freshOrder = db.orders.find(o => o.id === orderId) || order;
