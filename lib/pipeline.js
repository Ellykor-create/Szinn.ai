...
const { calcBirthChart, calcMonthlyTransits } = require('./astro');
...
function buildContext(order) {
  ...
  chart.transits = calcMonthlyTransits(chart, numerology.personalMonths);
  return { intake, chart, numerology };
}

// Genereert teksten + HTML voor één taal, met audit
async function generateLanguage(ctx, lang, client, addendum) {
  const { texts, usage } = await generateBlueprintTexts(ctx, lang, client, { addendum });
  const tAudit = auditTexts(texts, { aspects: (ctx.chart.aspects || []).length });
  if (!tAudit.ok) throw new Error(`tekst-audit ${lang}: ${tAudit.problems.join('; ')}`);
  const html = renderBlueprint({ ...ctx, ai: texts, lang });
  // De eigen intake meesturen zodat de audit een echte "Barry" in de klantdata
  // niet aanziet voor een lek van de demo-referentieklant.
  const inputText = [ctx.intake.clientName, ctx.intake.birthName, JSON.stringify(ctx.intake.raw || {})].join(' ');
  const hAudit = auditHTML(html, { clientName: ctx.intake.clientName, birthDate: ctx.intake.birthDate, inputText });
...
async function runGeneration(orderId, deps) {
  ...
  // Aanvullende prompt-instructies uit het admin-paneel (gelden voor alle
  // volgende generaties, tot ze in het paneel worden aangepast of geleegd).
  const addendum = String((db.settings && db.settings.promptAddendum) || '').trim();
  if (addendum) console.log(`${orderId}: admin-promptaanscherping actief (${addendum.length} tekens)`);
  ...
  for (let attempt = 1; attempt <= MAX_ATTEMPTS && !result; attempt++) {
    try {
      const nl = await generateLanguage(ctx, 'nl', null, addendum);
      console.log(`${orderId}: NL-teksten ok (poging ${attempt}; ${JSON.stringify(nl.usage || {})})`);
      const en = await generateLanguage(ctx, 'en', null, addendum);
