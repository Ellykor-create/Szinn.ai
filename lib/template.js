...
const L = {
  nl: {
    ...
    planetNames: { sun:'Zon', moon:'Maan', mercury:'Mercurius', venus:'Venus', mars:'Mars', jupiter:'Jupiter', saturn:'Saturnus', uranus:'Uranus', neptune:'Neptunus', pluto:'Pluto', northNode:'Noordknoop', southNode:'Zuidknoop', chiron:'Chiron', ascendant:'Ascendant', mc:'MC Midhemel' },
    elementNames: { vuur:'vuur', aarde:'aarde', lucht:'lucht', water:'water' },
    elementIntro: (counts) => `Dit is de snelste manier om je eigen kaart te lezen. De telling hieronder gaat over twaalf punten: de tien planeten plus je Ascendant en je Midhemel. Jouw verdeling is ${counts}.`,
    ascNote: (p, minPerDeg, flipTime, flipDir, flipSign, birthTime) =>
      `Over je geboortetijd. Je Ascendant staat op ${p}, dicht bij een tekenovergang. Voor dit tijdstip en deze plek is de snelheid van de Ascendant nagerekend: ongeveer één graad per ${minPerDeg} minuten. Bij een geboortetijd van ${flipTime} of ${flipDir} staat je Ascendant in ${flipSign} en verschuiven alle twaalf huizen mee. Klopt ${birthTime} met je geboorteakte, dan staat alles hierin.`,
    earlier: 'eerder', later: 'later',
  },
  en: {
    ...
    planetNames: { sun:'Sun', moon:'Moon', mercury:'Mercury', venus:'Venus', mars:'Mars', jupiter:'Jupiter', saturn:'Saturn', uranus:'Uranus', neptune:'Neptune', pluto:'Pluto', northNode:'North Node', southNode:'South Node', chiron:'Chiron', ascendant:'Ascendant', mc:'MC Midheaven' },
    elementNames: { vuur:'fire', aarde:'earth', lucht:'air', water:'water' },
    elementIntro: (counts) => `This is the quickest way to read your own chart. The count below covers twelve points: the ten planets plus your Ascendant and your Midheaven. Your distribution is ${counts}.`,
    ascNote: (p, minPerDeg, flipTime, flipDir, flipSign, birthTime) =>
      `About your birth time. Your Ascendant sits at ${p}, close to a sign boundary. For this time and place the Ascendant's speed has been calculated: roughly one degree per ${minPerDeg} minutes. With a birth time of ${flipTime} or ${flipDir}, your Ascendant moves into ${flipSign} and all twelve houses shift with it. If ${birthTime} matches your birth certificate, everything here stands.`,
    earlier: 'earlier', later: 'later',
  },
};

const PLANET_SYMBOLS = { sun:'☉', moon:'☽', mercury:'☿', venus:'♀', mars:'♂', jupiter:'♃', saturn:'♄', uranus:'⛢', neptune:'♆', pluto:'♇', northNode:'☊', southNode:'☋', chiron:'⚷', ascendant:'', mc:'' };
const ASPECT_NAMES_EN = { conjunct:'conjunct', sextiel:'sextile', vierkant:'square', driehoek:'trine', oppositie:'opposite' };
...
function renderBlueprint(ctx) {
  ...
  const tokens = {
    ...
    ASPECT_ROWS: buildAspectRows(chart, ai, t, lang),
    ASC_NOTE: buildAscNote(chart, intake, t, lang),
    ELEMENT_INTRO: buildElementIntro(chart, t, lang),
    MODALITY_CARD: ai.energy.modality && ai.energy.modality.body
      ? ncard('', ai.energy.modality.title, ai.energy.modality.body) : '',
    AI_CAL_INTRO: ai.calendar && ai.calendar.intro
      ? limitedHTML(ai.calendar.intro)
      : (lang === 'en'
        ? `<p>Your personal month numbers are calculated from Personal Year ${num.personalYear}. Each month carries its own colour of energy.</p>`
        : `<p>Je persoonlijke maandgetallen zijn berekend vanuit Persoonlijk Jaar ${num.personalYear}. Elke maand draagt een eigen kleur van energie.</p>`),
    ...
    CALENDAR_MONTHS: buildCalendar(num.personalMonths, t, lang, ai.calendar),
  };
...
function buildPlanetRows(P, ai, t, lang) {
  const order = ['sun','moon','mercury','venus','mars','jupiter','saturn','uranus','neptune','pluto','northNode','southNode','chiron','mc'];
  return order.map(key => {
    const p = P[key];
    if (!p || p.sign === '?') return '';
    // Knopen: geen Rx-badge — de ware knoop wisselt van richting en dat wordt
    // apart geduid, niet als retrograde-planeet gepresenteerd.
    const rx = p.retrograde && !['northNode', 'southNode'].includes(key) ? ' <span class="rx">Rx</span>' : '';
    const quality = limitedHTML((ai.astrology.qualities || {})[key] || '');
    const sym = PLANET_SYMBOLS[key] ? `${PLANET_SYMBOLS[key]} ` : '';
    return `\n      <tr><td>${sym}${t.planetNames[key]}${rx}</td><td>${posLang(p, lang)}</td><td>${p.house || ''}</td><td>${quality}</td></tr>`;
  }).join('');
}

function buildAspectRows(chart, ai, t, lang) {
  const aspects = chart.aspects || [];
  const texts = (ai.astrology && ai.astrology.aspects) || [];
  return aspects.map((a, i) => {
    const nm = (k) => `${PLANET_SYMBOLS[k] ? PLANET_SYMBOLS[k] + ' ' : ''}${t.planetNames[k] || k}`;
    const type = lang === 'en' ? (ASPECT_NAMES_EN[a.type] || a.type) : a.type;
    const orb = lang === 'en' ? `${a.orb}°` : `${String(a.orb).replace('.', ',')}°`;
    return `\n      <tr><td>${nm(a.a)} ${type} ${nm(a.b)}</td><td>${orb}</td><td>${limitedHTML(texts[i] || '')}</td></tr>`;
  }).join('');
}

// Geboortetijd-notitie: alleen wanneer de ASC binnen 3° van een tekengrens
// staat. Volledig deterministisch berekend; de AI komt hier niet aan te pas.
function buildAscNote(chart, intake, t, lang) {
  const s = chart.ascShift;
  const asc = chart.planets.ascendant;
  if (!s || !asc || asc.sign === '?') return '';
  const nearStart = s.degIntoSign < 3;
  const nearEnd = (30 - s.degIntoSign) < 3;
  if (!nearStart && !nearEnd) return '';
  const birthTime = intake.birthTime || '12:00';
  const [bh, bm] = birthTime.split(':').map(Number);
  const shiftMin = Math.ceil(nearStart ? s.minutesToPrevSign : s.minutesToNextSign);
  const totalMin = bh * 60 + bm + (nearStart ? -shiftMin : shiftMin);
  const flipTime = `${String(Math.floor(((totalMin % 1440) + 1440) % 1440 / 60)).padStart(2, '0')}:${String(((totalMin % 60) + 60) % 60).padStart(2, '0')}`;
  const flipSign = lang === 'en' ? (nearStart ? s.prevSignEn : s.nextSignEn) : (nearStart ? s.prevSign : s.nextSign);
  const minPerDegRaw = Math.round(s.minutesPerDeg * 10) / 10;
  const minPerDeg = lang === 'en' ? String(minPerDegRaw) : String(minPerDegRaw).replace('.', ',');
  return `<p>${t.ascNote(posLang(asc, lang), minPerDeg, flipTime, nearStart ? t.earlier : t.later, flipSign, birthTime)}</p>`;
}

function buildElementIntro(chart, t, lang) {
  const dist = chart.distribution;
  if (!dist) return '';
  const sorted = Object.entries(dist.elements).sort((a, b) => b[1].count - a[1].count);
  const counts = sorted.map(([el, v]) => `${v.count} ${t.elementNames[el]}`);
  const glue = lang === 'en' ? ' and ' : ' en ';
  const list = counts.length > 1 ? counts.slice(0, -1).join(', ') + glue + counts[counts.length - 1] : counts.join('');
  return `<p>${t.elementIntro(list)}</p>`;
}
...
function buildCalendar(personalMonths, t, lang, aiCal) {
  const monthNames = MONTHS[lang] || MONTHS.nl;
  const texts = CAL_TEXTS[lang] || CAL_TEXTS.nl;
  const aiMonths = (aiCal && Array.isArray(aiCal.months) && aiCal.months.length === (personalMonths || []).length)
    ? aiCal.months : null;
  return (personalMonths || []).map((m, i) => {
    // AI-geschreven maandtekst (met transitduiding) waar beschikbaar;
    // de vaste teksten per maandgetal blijven het vangnet (o.a. demo).
    const fallback = texts[m.number] || texts[9];
    const info = aiMonths ? {
      theme: limitedHTML(aiMonths[i].theme || fallback.theme),
      body: limitedHTML(aiMonths[i].body || fallback.body),
      bullets: (aiMonths[i].bullets || []).length === 3 ? aiMonths[i].bullets.map(b => limitedHTML(b)) : fallback.bullets,
    } : fallback;
